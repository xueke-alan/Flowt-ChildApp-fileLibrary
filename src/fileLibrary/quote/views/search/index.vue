<template>
  <div class="mainSearch">
    <div class="searchConter" ref="inputBar">
      ...
      <n-input size="large" placeholder=" 搜索报价单" class="inputBar" :class="{ focused: searchTipsOption.length }"
        @input="handleInput" @blur.stop="handleBlur" @focus="handleInput(inputValue)" @keyup="handleKeyUp"
        @keydown="handleKeyDown" v-model:value="inputValue" maxlength="30" :status="inputStatus">
        <template #prefix>
      ...
      <div class="searchTipsCont">
        <div class="note">
          <n-button class="error" v-if="inputStatus == 'error'" text type="error">
        ...
        <div class="tagGroup" v-if="!searchTipsOption.length">
          ...
          <n-popover trigger="click" placement="bottom" :width="300">
            ...
            <div>
              <n-input-group>
                <n-input size="small" placeholder="搜索内容" :style="{ flex: 2 }">
                </n-input>
                <n-select size="small" :style="{ flex: 1 }" placeholder="精确" />
              </n-input-group>
            </div>
            <template #footer>
              <div style="display: flex; gap: 5px">
                <n-popselect trigger="click">
                  <n-button size="small" secondary type="tertiary">
                    <n-icon size="20">
                      <component :is="Sticker24Regular"></component>
                    </n-icon></n-button>
                </n-popselect>
        ...
        <n-list class="searchTips" :class="{ hide: searchTipsOption.length }" bordered @click="searchPush()">
          <n-list-item v-for="(i, index) in searchTipsOption" class="searchTipsItem"
            :class="{ selected: index == optionSelected }">
            {{ i.lable }}
            <template #prefix> </template>
            <template #suffix>
              <n-tag round :bordered="false" type="success" style="
                  padding-left: 20px;
                  padding-right: 15px;
                  pointer-events: none;
                " v-for="t in i.suffix">
...
const handleInput = debounce(async (v: string) => {
  ...
  if (v) {
    ...
    if (inputValue.value) {
      ...
      // 查看高度，除以单个高度，得到数量
      // const restHeight = 200
      // const num = restHeight / 45
      // const targetElement = document.getElementById("your-element-id");


      const distanceToBottom = getElementBottomDistance(inputBar.value);
      console.log(`元素底部距离页面底部的距离：${distanceToBottom}px`);
      const num = Math.min(Math.max(Math.round(distanceToBottom / 45) - 3, 3), 8)

      searchTipsOption.value = queryList.slice(0, num);
    }
...
function getElementBottomDistance(element) {
...
.inputBar {
  ...
  .suffixIcon {
    ...
    &.spinner {
      animation: rotate 0.6s linear infinite, fadeIn 0.5s ease;
      /* 可以根据需要更改旋转时间 */
    }
  }
